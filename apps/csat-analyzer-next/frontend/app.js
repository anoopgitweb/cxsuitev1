...
const state={step:0,base:null,lookup:null,coverage:null,mapping:{},customDimensions:[],rules:{target:85,scale:'5',satisfiedMin:4,neutralMin:3,minimumSample:10,weekStart:'Sun',fiscalYearStartMonth:1,sparrowPath:DEFAULT_SPARROW_MODEL_PATH},analysis:null,answers:[],sentimentAnswers:[]};
const steps=['Base File','File Ready','Coverage','Mapping','Custom Dimensions','Business Rules','Analysis','Results'];

const questions=[

 ['CSAT trend over time','Mean by week, rolling direction, slope and period change'],['Overall CSAT for the period','Mean, median, survey count, range and standard deviation'],['Highest and lowest managers','Manager CSAT, rank, volume and distance from overall'],['Highest and lowest agents','Agent CSAT, rank, volume and distance from overall'],['Manager improvement over time','Current versus previous period and trend direction'],['Manager decline over time','Negative movement, consistency and materiality'],['Agent improvement over time','Current versus previous performance and slope'],['Agent decline over time','Negative movement and decline consistency'],['Consistent manager performance','Standard deviation, range and coefficient of variation'],['Volatile manager performance','Dispersion, spikes and sudden movement'],['Consistent agent performance','Agent-level spread and stability'],['Volatile agent performance','Range, variability and outlier count'],['Highest and lowest week','Weekly CSAT, volume, rank and overall comparison'],['Unusual spikes or drops','Deviation from mean and outlier screening'],['Managers above the organization average','Manager result minus overall CSAT'],['Agents above the organization average','Agent result minus overall CSAT'],['CSAT distribution by manager','Mean, quartiles, spread and skew indicators'],['CSAT distribution by agent','Mean, quartiles, spread and skew indicators'],['Manager performance gap','Best versus weakest result and team range'],['Balanced team performance','Narrow spread and similar agent outcomes'],['Top 10 agents','CSAT, volume and rank reliability'],['Bottom 10 agents','CSAT, volume and negative deviation'],['Agents improving month over month','Period movement and consistency count'],['Agents needing coaching','Below target, recent movement, volatility and volume'],['Managers needing attention','Low mean, decline, team spread and below-target share'],['Agents above target','Count and percentage at or above target'],['Agents below target','Count and percentage below target'],['Manager with most high-performing agents','Team average and agents above target'],['Leadership highlights and concerns','Target gap, movers, volatility, outliers and confidence']
...
questions.splice(28,1,
 ...
 ['Which managers have the highest confidence in their CSAT performance?','Sample size and confidence interval width'],
 ...
 ['Which managers remained above average throughout the period?','Repeated period comparison against organizational CSAT'],

 ['Which agents remained above average throughout the period?','Repeated period comparison against organizational CSAT'],
...
const sentimentQuestions=[
 ...
 [37,'How closely does customer sentiment align with the CSAT band?','Compare sentiment labels with the CSAT band for each response: Satisfied, Neutral, or Dissatisfied.','Spearman Correlation, Pearson Correlation, Cross-tab, p-value','Sentiment is read from the verbatim. CSAT band is used only as a validation signal.'],
 [38,'How often does customer sentiment disagree with the CSAT band?','Define mismatch rules using Satisfied, Neutral, and Dissatisfied bands. Example: Satisfied with negative sentiment, or Dissatisfied with positive sentiment.','Mismatch Count, Mismatch %, Cross-tab, Chi-square','Report mismatch rate overall and by period; do not treat mismatch as an error automatically.'],
 [39,'Which agents have the strongest alignment between sentiment and CSAT bands?','Calculate agent-level agreement between sentiment category and Satisfied, Neutral, or Dissatisfied band.','Agreement %, Cohen Kappa, Correlation, Count','Rank only agents with adequate matched sentiment and CSAT-band records.'],
 [40,'Which managers have the strongest alignment between sentiment and CSAT bands?','Calculate manager-level agreement between sentiment category and Satisfied, Neutral, or Dissatisfied band.','Agreement %, Cohen Kappa, Correlation, Count','Higher agreement means customer language and CSAT band tell a similar story.'],
 [41,'Which agents have the highest number of sentiment-to-CSAT-band mismatches?','Count and rate sentiment-to-CSAT-band mismatches by agent; use percentage to avoid volume bias.','Mismatch Count, Mismatch %, Total Count, CI','Prioritize high mismatch percentage only when matched record volume is adequate.'],
 [42,'Which managers have the highest number of sentiment-to-CSAT-band mismatches?','Count and rate sentiment-to-CSAT-band mismatches by manager; separate volume from rate.','Mismatch Count, Mismatch %, Total Count, CI','Use mismatch percentage for fair comparison and validate with verbatim examples.'],
 [43,'Is the relationship between sentiment and CSAT bands strengthening or weakening over time?','Calculate correlation/agreement between sentiment and CSAT band by period and track its direction.','Period Correlation, Period Kappa, Trend Slope, R-squared','Strengthening means the agreement metric increases over time; it does not prove causality.'],
...
function updateGuideForScreen(eyebrow='',title='',description=''){
  ...
  if(e.includes('begin'))return guideSay('Welcome. Choose the Base File that has one row per customer response. It should include a CSAT score or satisfaction label. Date is strongly recommended because it unlocks weekly, monthly, and fiscal-quarter trends. Agent and Manager fields unlock people and team views. Feedback/verbatim is optional for score-only analysis, but needed if you want sentiment or themes. Click Choose Base File when you are ready.','hint','#chooseBase');
  ...
  if(e.includes('confirm')||t.includes('mapped'))return guideSay('Column mapping guide. Feedback / verbatim: map the comment column when you want sentiment or themes. CSAT score: map the numeric rating used for averages and score trends. Satisfaction label: map only if you have text labels such as Satisfied, Neutral, or Dissatisfied. Response date: map this for weekly, monthly, and fiscal-quarter trends. Agent and Manager / team: map these for people and team insights. Wave / period and Tenure are optional, but useful for segmentation. Correct any dropdown, then click Confirm mapping.','hint','#confirmMapping');
  if(e.includes('business')||e.includes('final setup')||t.includes('rules'))return guideSay('Final setup guide. CSAT target: enter the official CSAT goal; this drives target gap and leadership readouts. Score scale: choose the scale used in the file, such as 1 to 5 or 1 to 10. Satisfied starts at: choose the lowest score that counts as satisfied. Neutral starts at: choose the lowest score that counts as neutral; lower scores are dissatisfied. Minimum sample for ranking: set the minimum records needed before agents, managers, periods, or segments are ranked. Week starts on: choose the first day of the account reporting week so weekly trends line up correctly. Fiscal year starts in: choose the client fiscal year start month when quarters do not follow the calendar year. Sparrow sentiment: turn it on only when verbatims have useful customer comments and you want tone added. Owl themes: turn it on when you want themes, drivers, issue types, or root-cause categories from comments. Note: local CSAT calculations always run; leave Sparrow and Owl off when comments are mostly blank or score-only analysis is enough.','hint','#run');
  ...
  if(e.includes('starting analysis'))return guideSay('Starting analysis. I am locking your mappings, CSAT rules, calendar settings, selected engines, and lookup enrichment for this run. Next I create the run log, then hand the job to the local analyzer. The live progress page will open automatically.','working');
  if(e.includes('analysis in progress')){const sentiment=state.rules.sparrow?'Sparrow sentiment model':'Local Rules',themes=state.rules.vulture?'Owl theme model':'Local Rules';return guideSay(`Live analysis guide. Core CSAT calculations are running locally on this computer. Sentiment analysis: ${sentiment}. Theme classification: ${themes}. I am validating required fields, processing records, calculating leadership questions, building evidence tables, and preparing readouts. Keep this page open and watch the stage, rows processed, elapsed time, and estimated remaining time.`,'working')}
...
function guideDashboardHandoff(){try{sessionStorage.setItem('csatDashboardGuideTour','1')}catch{}guideSay('I am opening the Detailed Dashboard now. This is the visual dashboard for deeper review, leadership storytelling, trend views, and export-ready summaries.','success')}

function openDetailedDashboard(){guideDashboardHandoff();setTimeout(()=>{const opened=window.open('/apps/csat-intelligence-hub/index.html?view=executive&tour=1','_blank');if(opened)opened.focus();else window.location.href='/apps/csat-intelligence-hub/index.html?view=executive&tour=1'},450)}
...
function showCoverage(){state.step=2;setScreen('ONE QUICK CHECK','Does the Base File already have all required fields?','You need a score or CSAT result, a response date for trends, and the business fields you want to compare. Feedback text is optional unless you plan to use sentiment or themes.',`<div class="choice-grid coverage-choice-grid"><button class="choice decision-choice yes-choice" id="complete"><span class="choice-motion" aria-hidden="true"><i></i><i></i><i></i></span><strong>Yes, everything is included</strong><span>Continue directly to column mapping.</span></button><button class="choice decision-choice enrich-choice" id="lookup"><span class="choice-motion" aria-hidden="true"><i></i><i></i><i></i></span><strong>No, I need to enrich it</strong><span>Add a lookup file and connect both files using a unique key.</span></button></div>`);$('complete').onclick=()=>{state.coverage='complete';showMapping()};$('lookup').onclick=()=>{state.coverage='lookup';showLookup()}}
...
function updateMappingGuide(){const score=document.querySelector('[data-map="score"]')?.value||'',csatCategory=document.querySelector('[data-map="satisfaction"]')?.value||'';if(score&&csatCategory&&score===csatCategory)return guideSay('Check the two CSAT outcome mappings. Both currently point to the same column, so the app cannot tell whether that field should be read as a number or as a label. Use CSAT Score only for raw numeric ratings such as 1 to 5, 1 to 10, or 0 to 100. Use CSAT Category only for text labels such as Satisfied, Neutral, or Dissatisfied. If this column is numeric, keep it under CSAT Score and clear CSAT Category unless a separate label column exists. If this column is text, keep it under CSAT Category and clear CSAT Score.','hint','#confirmMapping');if(score&&csatCategory)return guideSay('Both CSAT Score and CSAT Category are selected. This is useful only when they are two separate valid columns. CSAT Score keeps the raw rating for averages, target gap, trends, distributions, rankings, and statistical evidence. CSAT Category tells the app the final satisfaction band when the source file already has Satisfied, Neutral, or Dissatisfied. If a category value is blank or not recognized, the engine falls back to the numeric score and the Business Rules thresholds. Before continuing, confirm these two dropdowns are not pointing to duplicate or conflicting fields.','hint','#confirmMapping');if(score)return guideSay('CSAT Score is selected. Choose this when the field contains the raw numeric rating for each survey, such as 1 to 5, 1 to 10, or 0 to 100. This column drives CSAT average, target gap, weekly and monthly trends, fiscal-quarter views, agent and manager rankings, distributions, confidence checks, and the Results interpretation. Do not map a text status field here. In Business Rules, confirm Score scale, Satisfied starts at, and Neutral starts at so the app can convert each rating into Satisfied, Neutral, or Dissatisfied. A separate CSAT Category is optional only when the file already has a clean label column.','hint','#confirmMapping');if(csatCategory)return guideSay('Only CSAT Category is selected. Use this when the file has satisfaction labels such as Satisfied, Neutral, or Dissatisfied but no reliable numeric rating column. The app can calculate satisfaction share and band-based results from the labels. Numeric CSAT average, score distribution, score movement, and some statistical readouts will be limited unless a separate numeric CSAT Score is mapped. Check that the category values are consistently spelled and not mixed with numbers.','hint','#confirmMapping');return guideSay('Map the CSAT outcome. Select CSAT Score when you have a numeric rating column. Select CSAT Category when you have a text label column. Select both only when they are separate columns and both are meaningful. At least one outcome field is required before analysis can begin. Best practice is to map CSAT Score first, then add CSAT Category only if the file already provides a clean satisfaction label.','hint','#confirmMapping')}function showMapping(){state.step=3;const fields=[['feedback','Feedback / verbatim'],['score','CSAT score'],['satisfaction','Satisfaction label'],['date','Response date'],['agent','Agent'],['manager','Manager / team'],['wave','Wave / period'],['tenure','Tenure']];setScreen('PLEASE CONFIRM','I mapped the columns I could identify.','Review each selection once. Feedback and either a score or satisfaction field are required by the local engine; date, agent, and manager unlock stronger trend and people intelligence.',`<div class="mapping-grid">${fields.map(([key,label])=>`<div class="field-row"><label>${label}</label><select data-map="${key}">${options(analysisColumns(),state.mapping[key]||bestMap(key))}</select></div>`).join('')}</div>`);document.querySelectorAll('[data-map="score"],[data-map="satisfaction"]').forEach(select=>select.onchange=updateMappingGuide);updateMappingGuide();$('confirmMapping').onclick=()=>{document.querySelectorAll('[data-map]').forEach(s=>state.mapping[s.dataset.map]=s.value);if(!state.mapping.feedback){fail('Please map the feedback or comment column used by the local analysis engine.');return}if(!state.mapping.score&&!state.mapping.satisfaction){fail('Please map either a CSAT score or a Satisfaction label before continuing.');return}showCustomDimensions()};const backButton=$('back');if(backButton)backButton.onclick=showCoverage}

function showRules(){state.step=5;const sparrowPath=state.rules.sparrowPath||'models/sparrow_cnx_sentimentmodel';setScreen('FINAL SETUP','Confirm the business rules.','These settings determine how scores become outcomes and how much evidence is required before a person or team can be ranked. Sparrow and Owl are optional add-ons for verbatim-based intelligence.',`<div class="rule-grid"><div class="rule-card"><label>CSAT target</label><input id="target" type="number" value="${state.rules.target}"></div><div class="rule-card"><label>Score scale</label><select id="scale"><option value="5">1 to 5</option><option value="10">1 to 10</option><option value="custom">Custom</option></select></div><div class="rule-card"><label>Satisfied starts at</label><input id="satMin" type="number" value="${state.rules.satisfiedMin}"></div><div class="rule-card"><label>Neutral starts at</label><input id="neutralMin" type="number" value="${state.rules.neutralMin}"></div><div class="rule-card"><label>Minimum sample for ranking</label><input id="minimumSample" type="number" min="2" max="500" value="${state.rules.minimumSample||10}"></div><div class="rule-card"><label>Week starts on</label><select id="weekStart">${weekStartOptions(state.rules.weekStart||'Sun')}</select></div><div class="rule-card"><label>Fiscal year starts in</label><select id="fiscalYearStartMonth">${fiscalMonthOptions(state.rules.fiscalYearStartMonth||1)}</select></div></div><div class="action-panel signal-insights-panel"><span class="eyebrow">OPTIONAL INTELLIGENCE ENGINES</span><div class="engine-list"><div class="engine-option engine-option-with-path"><label class="engine-choice"><input type="checkbox" id="sparrow" ${state.rules.sparrow?'checked':''}> <span><strong>Sparrow sentiment</strong><br><small>Select when comments have meaningful customer language and you want Positive, Neutral, or Negative tone. Skip if verbatims are mostly blank or you only need score trends.</small><em>Best for sentiment briefing, emotional risk, and QA tone checks.</em></span></label><div class="model-path-field ${state.rules.sparrow?'':'hidden'}" id="sparrowPathField"><label for="sparrowPath">Sparrow model path</label><input id="sparrowPath" type="text" value="${escapeHtml(sparrowPath)}" spellcheck="false"><small class="model-path-help">Use the default path unless your Sparrow model is stored elsewhere.</small></div></div><label class="engine-option"><input type="checkbox" id="vulture" ${state.rules.vulture?'checked':''}> <span><strong>Owl themes</strong><br><small>Select when you want themes, drivers, issue types, or root-cause categories from verbatims. Skip when comments are very short or score-only analysis is enough.</small><em>Best for explaining why scores changed and where operations should act.</em></span></label></div></div>`);$('scale').value=state.rules.scale||'5';$('scale').onchange=()=>{const v=$('scale').value;if(v==='5'){$('satMin').value=4;$('neutralMin').value=3}else if(v==='10'){$('satMin').value=9;$('neutralMin').value=7}};$('sparrow').onchange=()=>{$('sparrowPathField').classList.toggle('hidden',!$('sparrow').checked)};document.querySelectorAll('#run').forEach(button=>button.onclick=prepareAnalysisStart);const backButton=$('back');if(backButton)backButton.onclick=showCustomDimensions}

function captureRulesFromScreen(){

  state.rules={target:Number(target?.value)||state.rules.target||85,scale:scale?.value||state.rules.scale||'5',satisfiedMin:Number(satMin?.value)||state.rules.satisfiedMin||4,neutralMin:Number(neutralMin?.value)||state.rules.neutralMin||3,minimumSample:Math.max(2,Number($('minimumSample')?.value)||state.rules.minimumSample||10),weekStart:String($('weekStart')?.value||state.rules.weekStart||'Sun'),fiscalYearStartMonth:Number($('fiscalYearStartMonth')?.value)||state.rules.fiscalYearStartMonth||1,sparrow:!!$('sparrow')?.checked,vulture:!!$('vulture')?.checked,sparrowPath:String($('sparrowPath')?.value||state.rules.sparrowPath||'models/sparrow_cnx_sentimentmodel').trim()}

}

function selectedEngineSummary(){

  const engines=['Local CSAT calculations'];

  if(state.rules.sparrow)engines.push('Sparrow sentiment');

  if(state.rules.vulture)engines.push('Owl themes');

  if(state.rules.acpt)engines.push('ACPT categorization');
...
function showAnalysisReady(){setScreen('READY TO ANALYZE','Ready to build your CSAT intelligence.','The analysis will start only after you click Start analysis on this page.',`<div class="analysis-ready-card"><div class="analysis-ready-intro"><span class="eyebrow">PRE-FLIGHT CHECK</span><strong>Everything needed for this run is ready.</strong><p>Selected engines: ${escapeHtml(selectedEngineSummary())}.</p></div><div class="analysis-ready-grid"><div><span>1</span><p>The analysis runs locally on this system.</p></div><div><span>2</span><p>Keep this window open until the run is complete.</p></div><div><span>3</span><p>I will show progress, rows processed, elapsed time, and ETA.</p></div><div><span>4</span><p>Sparrow, Owl, or ACPT may add processing time when selected.</p></div><div><span>5</span><p>A run log will be generated for review if anything fails.</p></div></div><div class="analysis-log-path analysis-preview-block"><span>Preview analysis File</span><div id="analysisFilePreview"><div class="live-note">Loading Preview analysis File...</div></div></div><div class="analysis-log-path"><span>Run log folder</span><code>&lt;App Folder&gt;\logs\analysis_runs\</code><small>Session logs, when enabled, are stored under <code>&lt;App Folder&gt;\logs\sessions\</code>.</small></div></div>`);document.querySelectorAll('#startConfirmed').forEach(button=>button.onclick=beginAnalysisHandoff);loadAnalysisFilePreview()}
...
async function runAnalysis(){state.step=6;showAnalysis();try{const payload={mode:'csat',mapping:{feedback:state.mapping.feedback||'',score:state.mapping.score||'',satisfaction:state.mapping.satisfaction||'',agent:state.mapping.agent||'',manager:state.mapping.manager||'',date:state.mapping.date||'',wave:state.mapping.wave||'',tenure:state.mapping.tenure||''},csatBands:{scale:state.rules.scale,satisfiedMin:state.rules.satisfiedMin,neutralMin:state.rules.neutralMin},engines:{sentiment:state.rules.sparrow?'sparrow':'local',theme:state.rules.vulture?'vulture':'local'},modelPaths:{sparrow:state.rules.sparrow?state.rules.sparrowPath:'',vulture:''},dynamicDimensions:state.customDimensions||[],calendar:{weekStart:state.rules.weekStart||'Sun',fiscalYearStartMonth:state.rules.fiscalYearStartMonth||1},baseKey:state.mapping.baseKey||'',lookupKey:state.mapping.lookupKey||''};const started=await post('/api/analyze',payload);await pollAnalysis(started.analysisId||started.analysis?.analysisId||'')}catch(e){showProcessError('Analysis',e,'Processing records')}}
...
function datasetEntityRows(kind){const rows=Array.isArray(state.analysis?.[kind])?state.analysis[kind]:[];return rows.map(r=>({name:nameOf(r),responses:num(r.Responses??r.responses??r.Count??r.count),csat:num(r.CSAT??r.csat??r['Agent CSAT']??r['Manager CSAT'])})).filter(r=>Number.isFinite(r.responses)||Number.isFinite(r.csat))}
...
function datasetSummaryTab(){const d=datasetSummaryData();const total=Number.isFinite(d.total)?d.total:0;const avgDay=Number.isFinite(d.dateInfo.days)&&d.dateInfo.days?total/d.dateInfo.days:NaN,avgWeek=Number.isFinite(d.dateInfo.weeks)&&d.dateInfo.weeks?total/d.dateInfo.weeks:NaN,avgMonth=Number.isFinite(d.dateInfo.months)&&d.dateInfo.months?total/d.dateInfo.months:NaN;const sections=[
 ...
 ['Model Information - Theme Classification', [['AI Model Used',state.rules.vulture?'Owl Theme Classification':'Local Rules'],['Model Type',state.rules.vulture?'Local Fine-Tuned Multi-Class Model':'Local rule-based engine (no AI model)'],['Model Version',state.rules.vulture?'Model package not available in this installation':'Not applicable'],['Model Size',state.rules.vulture?'Model package not available in this installation':'Not applicable'],['Training Dataset Version',state.rules.vulture?'Model package not available in this installation':'Not applicable'],['Date Last Trained',state.rules.vulture?'Model package not available in this installation':'Not applicable'],['Number of Training Samples',state.rules.vulture?'Model package not available in this installation':'Not applicable'],['Number of Output Classes',state.rules.vulture?'Model package not available in this installation':'Rule-based theme outputs']]],
 ...
 ['Data Quality Assessment', [['Records Successfully Processed',Number.isFinite(d.total)?Math.round(d.total).toLocaleString():'Not available'],['Records Excluded',Math.max(0,(state.base?.rows||0)-total).toLocaleString()],['Duplicate Records','Not flagged by current local profile'],['Missing Values',d.blankCells.toLocaleString()],['Missing Agent Names',profileMissing('agent').toLocaleString()],['Missing Manager Names',profileMissing('manager').toLocaleString()],['Missing CSAT Scores',profileMissing('score').toLocaleString()],['Missing Survey Dates',profileMissing('date').toLocaleString()],['Invalid CSAT Values','Validated during CSAT scoring'],['Invalid Date Formats','Invalid dates excluded from trend calculations']]],
...
function readoutMetrics(){
 ...
 const overall=num(summary.CSAT??summary.csat??summary.overallCSAT),total=num(summary.total??summary.Responses??summary.responses),target=num(state.rules.target);

 const periods=weekly.map(row=>({name:String(row.Week||row.Period||row.Date||''),csat:num(row.CSAT??row.csat),responses:num(row.Responses??row.responses)})).filter(row=>Number.isFinite(row.csat));

 const latest=periods.at(-1),previous=periods.at(-2),movement=latest&&previous?latest.csat-previous.csat:NaN;

 const values=periods.map(row=>row.csat),sd=std(values),range=values.length?Math.max(...values)-Math.min(...values):NaN;
 ...
 const sortEntity=(rows,key)=>rows.map(r=>({name:nameOf(r),csat:num(r[key]??r.CSAT??r.csat),responses:num(r.Responses??r.responses),trend:num(r.Movement??r.movement??r.Trend)})).filter(x=>Number.isFinite(x.csat)).sort((x,y)=>y.csat-x.csat);

 const ar=sortEntity(agents,'Agent CSAT'),mr=sortEntity(managers,'Manager CSAT');
...
function entityLine(entity,fallback){return entity?`${escapeHtml(entity.name)} (${fmt(entity.csat)} CSAT${Number.isFinite(entity.responses)?`, n=${Math.round(entity.responses).toLocaleString()}`:''})`:fallback}
...
function periodLine(period){return period?`${period.name||'Period'} at ${fmt(period.csat)} CSAT${Number.isFinite(period.responses)?` (n=${Math.round(period.responses).toLocaleString()})`:''}`:'Not enough dated periods'}
function entityRaw(entity,label){return entity?`${entity.name} ${fmt(entity.csat)} CSAT${Number.isFinite(entity.responses)?` (n=${Math.round(entity.responses).toLocaleString()})`:''}`:label}
const qaLensQuestionBank=[
...

 {
  ...
  "question": "Which agents generate the most satisfied?",

  "logic": "Count satisfied surveys by agent.",

  "considerations": "CSAT only. Apply minimum volume."
...

 {
  ...
  "question": "Which agents generate the most dissatisfied?",

  "logic": "Count dissatisfied surveys by agent.",
...

 {
  ...
  "question": "Which agents consistently receive satisfied comments?",
...

 {
  ...
  "logic": "Compare sentiment score with CSAT/CSAT.",
...

 {
  ...
  "logic": "Normalized (Survey Volume x Gap to Target x Dissatisfied Rate).",
...
function qaAgentScore(row){return num(qaAgentField(row,qaMappedNames('score',['Agent CSAT','CSAT','csat','CSAT Score','Score','score','CSAT','Agent CSAT'])))}
...
function qaAgentMovement(row){return num(qaAgentField(row,['Movement','movement','Change','change','CSAT Change','Trend','trend','Delta','delta']))}

function qaAgentSatisfied(row){return num(qaAgentField(row,['Satisfied','satisfied','Satisfied Count','Positive','Positive Count']))}

function qaAgentDissatisfied(row){return num(qaAgentField(row,['Dissatisfied','dissatisfied','Dissatisfied Count','Negative','Negative Count']))}

function qaCsatBand(row){

 const label=String(qaAgentField(row,qaMappedNames('satisfaction',['CSAT Type','CSAT Category','Category','Segment','segment']))||'').toLowerCase();

 const score=qaAgentScore(row),satisfiedMin=num(state.rules.satisfiedMin),neutralMin=num(state.rules.neutralMin);

 if(label.includes('satisfied'))return 'Satisfied';

 if(label.includes('dissatisfied'))return 'Dissatisfied';

 if(label.includes('neutral'))return 'Neutral';

 if(Number.isFinite(score)){if(score>=satisfiedMin)return 'Satisfied';if(score>=neutralMin)return 'Neutral';return 'Dissatisfied'}
...
function qaDerivedAgentsFromRows(){
 ...
 qaRawRows().forEach(row=>{const name=qaAgentName(row);if(!name||name==='Unmapped agent')return;const bucket=(groups[name] ||= {raw:{},name,score:NaN,responses:0,movement:NaN,satisfied:0,dissatisfied:0,neutral:0,values:[]});const score=qaAgentScore(row),band=qaCsatBand(row);bucket.responses+=1;if(Number.isFinite(score))bucket.values.push(score);if(band==='Satisfied')bucket.satisfied+=1;else if(band==='Dissatisfied')bucket.dissatisfied+=1;else if(band==='Neutral')bucket.neutral+=1});

 return Object.values(groups).map(row=>{const total=row.satisfied+row.neutral+row.dissatisfied;row.score=total?(row.satisfied/total*100-row.dissatisfied/total*100):(row.values.length?avg(row.values):NaN);return row});

}

function qaAgents(){const summary=(state.analysis?.agents||[]).map(row=>({raw:row,name:qaAgentName(row),score:qaAgentScore(row),responses:qaAgentResponses(row),movement:qaAgentMovement(row),satisfied:qaAgentSatisfied(row),dissatisfied:qaAgentDissatisfied(row)})).filter(row=>row.name&&row.name!=='Unmapped agent');const derived=qaDerivedAgentsFromRows();const byName={};derived.forEach(row=>byName[row.name]=row);summary.forEach(row=>{byName[row.name]={...(byName[row.name]||{}),...row,responses:Number.isFinite(row.responses)?row.responses:byName[row.name]?.responses,satisfied:Number.isFinite(row.satisfied)?row.satisfied:byName[row.name]?.satisfied,dissatisfied:Number.isFinite(row.dissatisfied)?row.dissatisfied:byName[row.name]?.dissatisfied,score:Number.isFinite(row.score)?row.score:byName[row.name]?.score}});return Object.values(byName).filter(row=>row.name)}
...
function qaBusinessImpactRows(agents,target){
 ...
 return agents.map(row=>{const gap=Number.isFinite(target)&&Number.isFinite(row.score)?Math.max(0,target-row.score):0,detRate=Number.isFinite(row.dissatisfied)&&Number.isFinite(row.responses)&&row.responses?row.dissatisfied/row.responses:0,value=(row.responses||0)*gap*(detRate||0);return {...row,value}}).filter(row=>Number.isFinite(row.value)&&row.value>0).sort((a,b)=>b.value-a.value);
...
function qaAnswerForQuestion(item){
 ...
 if(item.number===1||item.number===2){
  ...
  rows=qaBusinessImpactRows(agents,target);metric='Impact Score';formatter=value=>fmt(value);answer=rows.length?qaTopText(rows,metric,formatter):'No below-target high-volume dissatisfied concentration is available from mapped agent evidence.';
  ...
  rows=agents.filter(row=>Number.isFinite(row.satisfied)).map(row=>({...row,value:row.satisfied})).sort((a,b)=>b.value-a.value);metric='Satisfied';formatter=value=>Math.round(value).toLocaleString();answer=qaTopText(rows,metric,formatter);

 }else if(item.number===23){

  rows=agents.filter(row=>Number.isFinite(row.dissatisfied)).map(row=>({...row,value:row.dissatisfied})).sort((a,b)=>b.value-a.value);metric='Dissatisfied';formatter=value=>Math.round(value).toLocaleString();answer=qaTopText(rows,metric,formatter);
  ...
  rows=scoreRows.sort((a,b)=>b.score-a.score);metric='CSAT';answer=qaTopText(rows,metric,formatter);
...
const tlLensQuestionBank=[

 {
  ...
  "considerations": "Based on selected reporting period. Use Agent CSAT or Agent CSAT only, not QA score."
...

 {
  ...
  "considerations": "Current reporting period. Use CX target for CSAT/CSAT."
...

 {
  ...
  "logic": "Composite: missed target streak + gap to target + declining trend + dissatisfied rate",
...

 {
  ...
  "logic": "Lowest standard deviation of Agent CSAT/CSAT across periods",
...

 {
  ...
  "logic": "Highest standard deviation of Agent CSAT/CSAT across periods",
...

 {
  ...
  "logic": "Count satisfied responses by agent or satisfied rate",

  "considerations": "CSAT only. Apply minimum survey volume threshold."
...

 {
  ...
  "logic": "Count dissatisfied responses by agent or dissatisfied rate",
...
const opsLensQuestionBank=[

 {
  ...
  "considerations": "Use selected reporting period. Team score should be team-level CSAT/CSAT."
...

 {
  ...
  "logic": "Average team CSAT/CSAT by TL",
...

 {
  ...
  "logic": "Composite: low team score + missed targets + declining trend + high dissatisfied rate",
...

 {
  ...
  "question": "Which teams generate the most satisfied?",

  "logic": "Satisfied count/rate by team",

  "considerations": "CSAT only. Apply minimum survey volume."
...

 {
  ...
  "question": "Which teams generate the most dissatisfied?",

  "logic": "Dissatisfied count/rate by team",
...

 {
  ...
  "logic": "Declining trend + rising dissatisfied rate + missed target streak",
...
const clientLensQuestionBank=[
 ...
 {"category":"Executive Health","number":1,"question":"Are we meeting the agreed CSAT/CSAT target?","logic":"Compare current score against contractual target.","considerations":"Use selected reporting period."},
 ...
 {"category":"Customer Experience","number":8,"question":"Which channels have the highest CSAT/CSAT?","logic":"Rank channels by average score.","considerations":"Minimum survey threshold."},

 {"category":"Customer Experience","number":9,"question":"Which regions have the highest CSAT/CSAT?","logic":"Rank regions by average score.","considerations":"Minimum survey threshold."},
 ...
 {"category":"Business Impact","number":15,"question":"Which issues contribute most to dissatisfied?","logic":"Rank complaint themes among dissatisfied.","considerations":"CSAT only."},
 ...
 {"category":"Business Impact","number":18,"question":"Which operational KPI correlates most with CSAT?","logic":"Highest Pearson correlation coefficient.","considerations":"Need operational metrics."},
 ...
 {"category":"Volume","number":22,"question":"Does survey volume influence CSAT?","logic":"Correlation between volume and score.","considerations":"Minimum observations."},

 {"category":"Segmentation","number":23,"question":"Which region contributes the most satisfied?","logic":"Count satisfied by region.","considerations":"CSAT only."},

 {"category":"Segmentation","number":24,"question":"Which region contributes the most dissatisfied?","logic":"Count dissatisfied by region.","considerations":"CSAT only."},
...
function managerScoreFromRow(row){return num(qaAgentField(row,['Manager CSAT','Team CSAT','CSAT','csat','Score','score','CSAT','Manager CSAT']))}

function managerRowsSummary(){const derived={};qaRawRows().forEach(row=>{const name=managerNameFromRow(row);if(!name||name==='Unmapped team')return;const bucket=(derived[name] ||= {name,score:NaN,responses:0,satisfied:0,dissatisfied:0,neutral:0,values:[]}),score=qaAgentScore(row),band=qaCsatBand(row);bucket.responses+=1;if(Number.isFinite(score))bucket.values.push(score);if(band==='Satisfied')bucket.satisfied+=1;else if(band==='Dissatisfied')bucket.dissatisfied+=1;else if(band==='Neutral')bucket.neutral+=1});const raw=Object.values(derived).map(row=>{const total=row.satisfied+row.neutral+row.dissatisfied;row.score=total?(row.satisfied/total*100-row.dissatisfied/total*100):(row.values.length?avg(row.values):NaN);return row});const summary=(state.analysis?.managers||[]).map(row=>{return {name:managerNameFromRow(row),score:managerScoreFromRow(row),responses:qaAgentResponses(row),satisfied:qaAgentSatisfied(row),dissatisfied:qaAgentDissatisfied(row)}}).filter(row=>row.name&&row.name!=='Unmapped team');const byName={};raw.forEach(row=>byName[row.name]=row);summary.forEach(row=>byName[row.name]={...(byName[row.name]||{}),...row,responses:Number.isFinite(row.responses)?row.responses:byName[row.name]?.responses,satisfied:Number.isFinite(row.satisfied)?row.satisfied:byName[row.name]?.satisfied,dissatisfied:Number.isFinite(row.dissatisfied)?row.dissatisfied:byName[row.name]?.dissatisfied,score:Number.isFinite(row.score)?row.score:byName[row.name]?.score});return Object.values(byName).filter(row=>row.name)}
...
function clientSegmentRows(names){
 ...
 qaRawRows().forEach(row=>{
  ...
  const bucket=(map[name]||={name,values:[],responses:0,satisfied:0,dissatisfied:0});

  const score=qaAgentScore(row),band=qaCsatBand(row);

  bucket.responses+=1;

  if(Number.isFinite(score))bucket.values.push(score);

  if(band==='Satisfied')bucket.satisfied+=1;

  else if(band==='Dissatisfied')bucket.dissatisfied+=1;

 });

 return Object.values(map).map(row=>{

  const derived=row.values.length?avg(row.values):(row.responses?(row.satisfied/row.responses*100-row.dissatisfied/row.responses*100):NaN);
...
function clientPeriodRows(){const periods=analysisPeriods().map(row=>({name:rowPeriodName(row)||row.name||row.label||'Period',score:num(row?.CSAT??row?.csat??row?.CSAT??row?.csat??row?.Score??row?.score),responses:rowResponses(row)})).filter(row=>Number.isFinite(row.score)||Number.isFinite(row.responses));return periods}
...
function answerClientLens(item){
 ...
 if(item.number===1){
  ...
  rows=clientThemeRows(item.number===11);metric=item.number===15?'Dissatisfied Issue Count':'Theme Mentions';formatter=value=>Math.round(value).toLocaleString();answer=rows.length?qaTopText(rows,metric,formatter):`The leading driver is ${reason.label}; theme output is limited in this run.`;
  ...
  rows=clientSegmentRows(['Region','Geo','Market','Country']).map(row=>({...row,value:row.satisfied})).sort((a,b)=>b.value-a.value);metric='Satisfied';formatter=value=>Math.round(value).toLocaleString();answer=rows.length?qaTopText(rows,metric,formatter):answer;

 }else if(item.number===24){

  rows=clientSegmentRows(['Region','Geo','Market','Country']).map(row=>({...row,value:row.dissatisfied})).sort((a,b)=>b.value-a.value);metric='Dissatisfied';formatter=value=>Math.round(value).toLocaleString();answer=rows.length?qaTopText(rows,metric,formatter):answer;
...
function vpRollingAvg(periods, count, offset=0){
 ...
 const slice=periods.slice(Math.max(0,periods.length-count-offset), periods.length-offset).map(row=>row.csat).filter(Number.isFinite);
...
function vpStreak(periods,target,met=true){
 ...
 periods.forEach(period=>{const ok=Number.isFinite(period.csat)&&(met?period.csat>=target:period.csat<target);current=ok?current+1:0;best=Math.max(best,current)});
...
function vpCsatBuckets(total){

 const a=state.analysis||{},summary=a.summary||{},counts=a.counts||{};

 return csatBucketCountsFromSummary(summary,counts,total);
...
function vpLensSections(){
 ...
 const weeksMet=Number.isFinite(target)?last12.filter(row=>Number.isFinite(row.csat)&&row.csat>=target).length:NaN;

 const targetPct=last12.length&&Number.isFinite(weeksMet)?weeksMet/last12.length*100:NaN;

 const deltas=last12.map(row=>Number.isFinite(row.csat)&&Number.isFinite(target)?row.csat-target:NaN).filter(Number.isFinite);
 ...
 const avg4=vpRollingAvg(periods,4),prev4=vpRollingAvg(periods,4,4),weeklyRate=periods.length>=2?(periods.at(-1).csat-periods[0].csat)/Math.max(1,periods.length-1):NaN;

 const best=periods.length?[...periods].sort((a,b)=>b.csat-a.csat)[0]:null,weak=periods.length?[...periods].sort((a,b)=>a.csat-b.csat)[0]:null;
 ...
 const corrVol=vpCorrelation(periods.map(p=>p.responses),periods.map(p=>p.csat));

 const buckets=vpCsatBuckets(total),satisfied=buckets.satisfied,neutral=buckets.neutral,dissatisfied=buckets.dissatisfied;

 const satisfiedPct=Number.isFinite(satisfied)&&Number.isFinite(total)&&total?satisfied/total*100:NaN;

 const neutralPct=Number.isFinite(neutral)&&Number.isFinite(total)&&total?neutral/total*100:NaN;

 const dissatisfiedPct=Number.isFinite(dissatisfied)&&Number.isFinite(total)&&total?dissatisfied/total*100:NaN;
 ...
 const satisfactionState=Number.isFinite(m.movement)?m.movement>0?'improving':m.movement<0?'declining':'stable':'needs another period';

 const sections=[

  {title:'Executive Summary',kpis:[['Current CSAT',fmt(m.overall)],['Target CSAT',vpLensValue(target)],['Variance to Target',Number.isFinite(m.gap)?`${signed(m.gap)} pts`:'n/a'],['Previous Period CSAT',prev?fmt(prev.csat):'n/a'],['Period Change',Number.isFinite(m.movement)?`${signed(m.movement)} pts`:'n/a'],['Survey Responses',vpLensNum(total)],['Response Rate','Not available']],readout:`Overall CSAT stands at ${fmt(m.overall)}, representing ${Number.isFinite(m.movement)?`${signed(m.movement)} point change`:'an unavailable period change'} from the previous reporting period and ${Number.isFinite(m.gap)?`${Math.abs(m.gap).toFixed(2)} points ${m.gap>=0?'above':'below'} target`:'no target variance available'}. Customer satisfaction is ${satisfactionState}.`},

  {title:'Performance Against Target',kpis:[['Weeks Meeting Target (Last 12 Weeks)',Number.isFinite(weeksMet)?String(weeksMet):'n/a'],['Target Achievement %',vpLensPct(targetPct)],['Average Delta to Target',Number.isFinite(avgDelta)?`${signed(avgDelta)} pts`:'n/a'],['Best Delta',Number.isFinite(bestDelta)?`${signed(bestDelta)} pts`:'n/a'],['Worst Delta',Number.isFinite(worstDelta)?`${signed(worstDelta)} pts`:'n/a'],['Longest Success Streak',String(vpStreak(last12,target,true))],['Longest Miss Streak',String(vpStreak(last12,target,false))]],readout:`The organization achieved its CSAT target in ${Number.isFinite(weeksMet)?weeksMet:'n/a'} of the last ${last12.length||12} weeks, maintaining an average performance of ${Number.isFinite(avgDelta)?`${Math.abs(avgDelta).toFixed(2)} points ${avgDelta>=0?'above':'below'} target`:'n/a'}. Target achievement is ${Number.isFinite(targetPct)?targetPct>=75?'consistent':targetPct>=40?'variable':'inconsistent':'not available'}.`},

  {title:'Trend & Momentum',kpis:[['Weekly Trend',m.trend],['Rolling 4-Week Average',fmt(avg4)],['Previous 4-Week Average',fmt(prev4)],['Weekly Improvement Rate',Number.isFinite(weeklyRate)?`${signed(weeklyRate)} pts/week`:'n/a'],['Trend Direction',m.trend]],readout:`CSAT momentum over the past four weeks indicates customer satisfaction is ${Number.isFinite(avg4)&&Number.isFinite(prev4)?avg4>=prev4?'accelerating or strengthening':'slowing or weakening':'not fully measurable'}, with an average movement of ${Number.isFinite(weeklyRate)?signed(weeklyRate):'n/a'} points per week. The overall trend suggests performance is ${m.trend.toLowerCase()}.`},

  {title:'Performance Stability',kpis:[['Standard Deviation',fmt(m.sd)],['Stability Rating',m.consistency],['Weekly Variability',m.variability],['Best Week',best?`${fmt(best.csat)} (${best.name})`:'n/a'],['Worst Week',weak?`${fmt(weak.csat)} (${weak.name})`:'n/a']],readout:`Customer experience performance remains ${m.consistency.toLowerCase()} / ${m.variability.toLowerCase()}, suggesting recent results are ${m.consistency==='High'?'consistently sustained':'subject to operational variation'}.`},

  {title:'Survey Volume & Statistical Confidence',kpis:[['Survey Responses',vpLensNum(total)],['Response Rate','Not available'],['Volume Trend',volumeDirection],['Average Weekly Volume',fmt(avgVolume)],['Highest Volume Week',highVol?`${vpLensNum(highVol.responses)} (${highVol.name})`:'n/a'],['Lowest Volume Week',lowVol?`${vpLensNum(lowVol.responses)} (${lowVol.name})`:'n/a'],['Margin of Error',Number.isFinite(margin)?`+/- ${fmt(margin)} pts`:'n/a'],['Confidence Level',m.confidence]],readout:`Survey participation has ${volumeDirection} over the reporting period. Current response levels provide ${m.confidence.toLowerCase()} statistical confidence that the reported CSAT represents customer sentiment.`},

  {title:'Correlation Analysis',kpis:[['Volume <-> CSAT',Number.isFinite(corrVol)?fmt(corrVol):'n/a'],['Volume <-> Satisfied','Not available'],['Volume <-> Dissatisfied','Not available'],['Response Rate <-> CSAT','Not available'],['Sentiment <-> CSAT',Number.isFinite(sent.positive)||Number.isFinite(sent.negative)?'Directional':'Not available'],['Operational Metric <-> CSAT','Depends on mapped fields']],readout:`Correlation analysis indicates survey participation ${Number.isFinite(corrVol)&&Math.abs(corrVol)>=0.5?'may materially influence':'does not clearly prove influence on'} CSAT performance. The strongest available relationship is volume to CSAT when weekly volume data exists.`},

  {title:'Customer Mix',kpis:[['Satisfied %',vpLensPct(satisfiedPct)],['Neutral %',vpLensPct(neutralPct)],['Dissatisfied %',vpLensPct(dissatisfiedPct)],['Satisfied Growth','Needs prior mix'],['Dissatisfied Growth','Needs prior mix']],readout:`The customer base comprises ${vpLensPct(satisfiedPct)} Satisfied, ${vpLensPct(neutralPct)} Neutral, and ${vpLensPct(dissatisfiedPct)} Dissatisfied. Changes in customer mix indicate overall satisfaction is ${satisfactionState}.`},
  ...
  {title:'Team Performance',kpis:[['Team Rankings',m.bestManager||m.weakManager?'Available':'Needs manager mapping'],['Team CSAT',m.bestManager?entityRaw(m.bestManager,'n/a'):'n/a'],['Team Improvement',m.mostImprovedManager?entityRaw(m.mostImprovedManager,'n/a'):'n/a'],['Target Achievement by Team','Use manager summary']],readout:`Performance varies across teams where manager mapping exists. ${m.bestManager?m.bestManager.name:'Top team'} is outperforming available peers while ${m.weakManager?m.weakManager.name:'the lowest team'} presents the largest opportunity for improvement.`},

  {title:'Agent Performance',kpis:[['Top Agents',m.bestAgent?entityRaw(m.bestAgent,'n/a'):'n/a'],['Bottom Agents',m.weakAgent?entityRaw(m.weakAgent,'n/a'):'n/a'],['Average Agent CSAT',state.analysis?.agents?.length?fmt(avg(state.analysis.agents.map(r=>num(r['Agent CSAT']??r.CSAT??r.csat)).filter(Number.isFinite))):'n/a'],['Performance Distribution',m.bestAgent&&m.weakAgent?'Available':'Needs agent mapping']],readout:`Individual performance remains ${m.consistency==='High'?'consistent':'variable'}, indicating opportunities for targeted coaching and adoption of best practices.`},

  {title:'Customer Segmentation',kpis:[['Region','Use Custom Views'],['Product','Use Custom Views'],['Channel','Use Custom Views'],['Business Unit','Use Custom Views'],['Customer Type','Use Custom Views']],readout:'Customer satisfaction may differ across business segments. Use Custom Views to compare region, product, channel, business unit, or customer type when those fields exist in the uploaded file.'},

  {title:'Dissatisfied Analysis',kpis:[['Top Complaint Categories',reason.label],['Complaint Trend',m.trend],['Repeat Issues',vpTopLabels(themes,3)],['Dissatisfied Concentration',vpLensPct(dissatisfiedPct)]],readout:`The majority of dissatisfied risk should be reviewed around ${reason.label} and ${vpTopLabels(themes,3)}, highlighting the areas with the greatest potential to improve satisfaction.`},

  {title:'Satisfied Analysis',kpis:[['Top Satisfaction Drivers',vpTopLabels(themes,3)],['Repeat Positive Themes',topPositive],['Strength Areas',m.bestAgent||m.bestManager?'Available':'Not mapped']],readout:`Customers recognize ${vpTopLabels(themes,3)} as recurring experience signals. Use satisfied verbatims to preserve and replicate these strengths.`},

  {title:'Risks',kpis:[['Rising Dissatisfied',Number.isFinite(dissatisfiedPct)?dissatisfiedPct>30?'Yes':'Monitor':'n/a'],['Falling Volume',Number.isFinite(latestVol)?latestVol<0?'Yes':'No':'n/a'],['Missed Targets',Number.isFinite(m.gap)?m.gap<0?'Yes':'No':'n/a'],['Performance Volatility',m.variability],['Emerging Negative Themes',vpTopLabels(themes,2)]],readout:`Current trends indicate ${m.outlook==='At Risk'?'high':m.outlook==='Neutral'?'moderate':'low'} risk to future CSAT performance, primarily driven by ${reason.label}, target variance, and volatility.`},

  {title:'Opportunities',kpis:[['Highest Impact Driver',reason.label],['Quick Wins',vpTopLabels(acpt,2)],['Estimated Improvement Potential',Number.isFinite(m.gap)&&m.gap<0?`${fmt(Math.abs(m.gap))} pts to target`:'Protect current level'],['High-Impact Segments','Use Custom Views']],readout:`The greatest opportunity to improve customer satisfaction lies in addressing ${reason.label}, while sustaining strengths in ${vpTopLabels(themes,2)}.`},

  {title:'Predictive Outlook',kpis:[['Forecast CSAT',Number.isFinite(m.overall)&&Number.isFinite(weeklyRate)?fmt(m.overall+weeklyRate):'n/a'],['Probability of Meeting Target',Number.isFinite(m.gap)?(m.gap>=0?'High':m.gap>=-3?'Moderate':'Low'):'n/a'],['Forecast Trend',m.trend],['Confidence Score',m.confidence]],readout:`Based on current trends, CSAT is projected to ${m.trend==='Improving'?'improve':m.trend==='Declining'?'decline':'remain stable'} over the next reporting period, with ${Number.isFinite(m.gap)?(m.gap>=0?'high':m.gap>=-3?'moderate':'low'):'unknown'} probability of meeting the established target.`}
...
function readoutCards(lens){const m=readoutMetrics(),reason=readoutReason();const weakest=m.periods.length?[...m.periods].sort((a,b)=>a.csat-b.csat)[0]:null;const action=lens==='qa'?'Audit dissatisfied/neutral records and ACPT/process tags':lens==='tl'?'Coach repeat low-score patterns with verbatim evidence':lens==='ops'?'Stabilize the weakest period and driver before broad coaching':lens==='client'?'Confirm recovery plan and next-period monitoring':'Protect the current business outcome and remove the biggest operating risk';return [

 {label:'Current Health',value:m.outlook,evidence:[`CSAT ${fmt(m.overall)}`,Number.isFinite(m.gap)?`Target gap ${signed(m.gap)} pts`:'No target gap',Number.isFinite(m.total)?`n=${Math.round(m.total).toLocaleString()}`:'n/a']},
...
function healthReadout(m,metric){const gapText=Number.isFinite(m.gap)?m.gap<0?`but it is ${fmt(Math.abs(m.gap))} pts below target`:m.gap>0?`and it is ${fmt(m.gap)} pts above target`:`and it is on target`:`with target gap not available`;const movementText=Number.isFinite(m.movement)?m.movement>0?`latest movement is up ${fmt(Math.abs(m.movement))} pts`:m.movement<0?`latest movement is down ${fmt(Math.abs(m.movement))} pts`:`latest movement is flat`:`latest movement needs another dated period`;return `The business is ${m.trend.toLowerCase()} with ${metric} at ${fmt(m.overall)}; ${gapText}, and ${movementText}.`}function lensLines(lens){if(lens==='vp')return vpLensInsightRows();if(lens==='qa')return qaLensInsightRows();const m=readoutMetrics(),reason=readoutReason(),sent=readoutSentiment();const weakestPeriod=m.periods.length?[...m.periods].sort((a,b)=>a.csat-b.csat)[0]:null;const bestPeriod=m.periods.length?[...m.periods].sort((a,b)=>b.csat-a.csat)[0]:null;const privateNames=lens!=='client';const tlAgent=privateNames?entityRaw(m.weakAgent,'Agent mapping not available'):'Agent-level detail withheld';const opsManager=privateNames?entityRaw(m.weakManager,'Manager/team mapping not available'):'Team-level detail withheld';const common=[

 {title:'Customer satisfaction readout',finding:healthReadout(m,'CSAT'),why:`Keep the discussion on the target gap, latest movement, and the action needed on ${reason.label}.`,evidence:[`CSAT ${fmt(m.overall)}`,Number.isFinite(m.gap)?`Target gap ${signed(m.gap)} pts`:'Target gap n/a',Number.isFinite(m.total)?`Responses ${Math.round(m.total).toLocaleString()}`:'Responses n/a']},
 ...
 {title:'Primary driver should anchor the action plan',finding:`The leading driver is ${reason.label}.`,why:'A driver-led action is more useful than a generic ï¿½improve CSATï¿½ message.',evidence:[`Driver ${reason.label}`,Number.isFinite(reason.count)?`Count ${Math.round(reason.count).toLocaleString()}`:'Count n/a',`Negative sentiment ${fmt(sent.negative)}%`]},
 ...
 qa:[
  ...
  {title:'QA should audit the lowest satisfaction slice first',finding:`Start with ${periodLine(weakestPeriod)} and dissatisfied/neutral records tied to ${reason.label}.`,why:'This creates a focused audit queue instead of a random sample.',evidence:[weakestPeriod?`Period ${weakestPeriod.name}`:'Period n/a',`Driver ${reason.label}`,`Negative ${fmt(sent.negative)}%`]},

  {title:'Audit for process evidence before coaching tone',finding:'Check resolution, documentation, policy, and handoff evidence before assigning behavior coaching.',why:'Low CSAT can come from process friction even when the agent tone is acceptable.',evidence:[`ACPT/theme evidence available`,Number.isFinite(m.total)?`Rows ${Math.round(m.total).toLocaleString()}`:'Rows n/a',`Driver ${reason.label}`]},
  ...
  {title:'Close the loop with a recovery check',finding:'After audit actions, compare the next completed period against the current weakest period.',why:'The readout should prove whether the fix changed customer satisfaction, not just whether action was taken.',evidence:[`Baseline ${weakestPeriod?weakestPeriod.name:'n/a'}`,`Current trend ${m.trend}`,`Confidence ${m.confidence}`]},

  {title:'QA summary for leadership',finding:`Audit priority is ${reason.label} in the lowest-scoring period.`,why:'This gives leadership a precise review queue with evidence.',evidence:[`CSAT ${fmt(m.overall)}`,`Risk ${m.outlook}`,`Sample ${Number.isFinite(m.total)?Math.round(m.total).toLocaleString():'n/a'}`]}

 ],

 tl:[

  {title:'Coaching should be evidence-led',finding:`Primary coaching candidate: ${tlAgent}.`,why:'The TL view should only name agents when the data supports a focused coaching conversation.',evidence:[tlAgent,`Overall CSAT ${fmt(m.overall)}`,`Minimum sample ${state.rules.minimumSample||10}`]},
 ...
 vp:[
  ...
  {title:'Executive message should be about business health',finding:`Customer satisfaction is ${m.outlook.toLowerCase()} with CSAT at ${fmt(m.overall)}.`,why:'A VP needs the decision state, not internal manager rankings.',evidence:[`CSAT ${fmt(m.overall)}`,Number.isFinite(m.gap)?`Target gap ${signed(m.gap)} pts`:'Target gap n/a',`Confidence ${m.confidence}`]},
 ...
 client:[
  ...
  {title:'Client narrative should be clean and external-ready',finding:`CSAT is ${fmt(m.overall)} with a ${m.outlook.toLowerCase()} outlook.`,why:'The client needs confidence, risk, and action without internal hierarchy noise.',evidence:[`CSAT ${fmt(m.overall)}`,`Responses ${Number.isFinite(m.total)?Math.round(m.total).toLocaleString():'n/a'}`,`Confidence ${m.confidence}`]},

  {title:'Customer-impact risk is the weakest period',finding:`The lowest customer satisfaction period is ${periodLine(weakestPeriod)}.`,why:'This is the simplest shared reference point for client discussion.',evidence:[weakestPeriod?`Weakest ${weakestPeriod.name}`:'Weakest n/a',bestPeriod?`Best ${bestPeriod.name}`:'Best n/a',`Range ${Number.isFinite(m.range)?fmt(m.range):'n/a'}`]},
...
function dimensionQuestionRows(field,groups,minSample){const reliable=groups.filter(g=>g.reliable),byScore=[...reliable].sort((a,b)=>b.avg-a.avg),byVolume=[...groups].sort((a,b)=>b.count-a.count),weak=[...reliable].sort((a,b)=>a.avg-b.avg),negative=[...groups].filter(g=>Number.isFinite(g.negativePct)).sort((a,b)=>b.negativePct-a.negativePct),spread=byScore.length>=2?byScore[0].avg-weak[0].avg:NaN,highVolumeLowScore=byVolume.filter(g=>g.reliable&&Number.isFinite(g.avg)).sort((a,b)=>b.count-a.count||a.avg-b.avg).find(g=>g.avg<(avg(reliable.map(x=>x.avg))||0));const q=(question,status,basis,formula,values)=>({question,status,basis,formula,values});return[
 q(`Which ${field} is performing best?`,byScore[0]?'yes':'unknown',byScore[0]?`${byScore[0].label} has the highest average CSAT among reliable ${field} groups.`:`No ${field} group has enough score evidence.`,'Rank reliable groups by average CSAT descending',byScore.slice(0,5).map(g=>({[field]:g.label,Responses:g.count,'Avg CSAT':g.avg}))),
 q(`Which ${field} is weakest?`,weak[0]?'no':'unknown',weak[0]?`${weak[0].label} has the lowest average CSAT among reliable ${field} groups.`:`No ${field} group has enough score evidence.`,'Rank reliable groups by average CSAT ascending',weak.slice(0,5).map(g=>({[field]:g.label,Responses:g.count,'Avg CSAT':g.avg}))),
 ...
 q(`Which ${field} has the highest volume?`,byVolume[0]?'yes':'unknown',byVolume[0]?`${byVolume[0].label} has the highest response volume.`:`No ${field} values are available.`,'Rank groups by response count descending',byVolume.slice(0,5).map(g=>({[field]:g.label,Responses:g.count,'Avg CSAT':g.avg}))),
 q(`Is any high-volume ${field} group below average?`,highVolumeLowScore?'no':'yes',highVolumeLowScore?`${highVolumeLowScore.label} has high volume and below-average CSAT.`:`No high-volume reliable ${field} group is below the dimension average.`,'Find reliable high-volume groups with average score below the reliable-group average',highVolumeLowScore?[{[field]:highVolumeLowScore.label,Responses:highVolumeLowScore.count,'Avg CSAT':highVolumeLowScore.avg,'Dimension Avg':avg(reliable.map(x=>x.avg))}]:[]),
...
function csatBucketCountsFromSummary(summary={},counts={},total=0){

 const exact=[num(counts.Satisfied??counts.Satisfied??counts.satisfied??counts.satisfied??counts.Satisfied_Count??counts.satisfiedCount),num(counts.Neutral??counts.Neutral??counts.neutral??counts.neutral??counts.Neutral_Count??counts.neutralCount),num(counts.Dissatisfied??counts.Dissatisfied??counts.dissatisfied??counts.dissatisfied??counts.Dissatisfied_Count??counts.dissatisfiedCount)];

 const exactTotal=exact.reduce((sum,value)=>sum+(Number.isFinite(value)?value:0),0);

 if(Number.isFinite(total)&&total>0&&exact.every(Number.isFinite)&&Math.abs(exactTotal-total)<=1){return {satisfied:exact[0],neutral:exact[1],dissatisfied:exact[2],satisfiedPct:exact[0]/total*100}}

 const supplied=[num(summary.Satisfied??summary.satisfied??summary.Satisfied??summary.satisfied??summary.satisfiedCount),num(summary.Neutral??summary.neutral??summary.Neutral??summary.neutral??summary.neutralCount),num(summary.Dissatisfied??summary.dissatisfied??summary.Dissatisfied??summary.dissatisfied??summary.dissatisfiedCount)];
 ...
 return {satisfied:values[0],neutral:values[1],dissatisfied:values[2],satisfiedPct:percentShaped?supplied[0]:(Number.isFinite(values[0])&&Number.isFinite(total)&&total?values[0]/total*100:NaN)};
...
function performanceOverviewTab(){
 ...
 const score=num(summary.CSAT??summary.csat??summary.overallCSAT??summary.score??a.overall?.csat);
 const bucketCounts=csatBucketCountsFromSummary(summary,a.counts||{},total),first=num(bucketCounts.satisfied),second=num(bucketCounts.neutral),third=num(bucketCounts.dissatisfied),pct=num(bucketCounts.satisfiedPct);
 const scoreRows=analysisPeriods().map(r=>({period:rowPeriodName(r),score:num(r.CSAT??r.csat??r.mean??r.Mean??r.score??r.Score),responses:rowResponses(r)})).filter(r=>Number.isFinite(r.score));
 ...
 const cards=[
  ...
  ['Satisfied',Number.isFinite(first)?Math.round(first).toLocaleString():'n/a','Records in this outcome band.','good'],
  ['Neutral',Number.isFinite(second)?Math.round(second).toLocaleString():'n/a','Records in the middle band.','neutral'],
  ['Dissatisfied',Number.isFinite(third)?Math.round(third).toLocaleString():'n/a','Records in the negative band.','risk'],
  ['% satisfied',Number.isFinite(pct)?fmt(pct)+'%':'n/a','Positive outcome share.','good'],
  ['CSAT',Number.isFinite(score)?fmt(score):'n/a','All records used in this CSAT calculation.','good'],
  ['WoW CSAT',Number.isFinite(movement)?signed(movement)+' pts':'n/a',latest&&prev?'Latest '+fmt(latest.score)+' vs prior '+fmt(prev.score)+'.':'Need two periods.',Number.isFinite(movement)&&movement<0?'risk':'good'],
  ...
  ['4-week avg CSAT',Number.isFinite(avg4)?fmt(avg4):'n/a','Rolling view across latest available weeks.','good'],
  ['Best week',best?fmt(best.score):'n/a',best?.period||'', 'good'],
  ['Lowest week',low?fmt(low.score):'n/a',low?.period||'', 'risk'],
  ['CSAT volatility',Number.isFinite(volatility)?fmt(volatility):'n/a','Lower is more consistent week to week.','neutral'],
...
function decisionGuideRows(){
 ...
 const a=state.analysis||{},summary=a.summary||{},sentiment=a.sentiment||{},total=num(summary.total??summary.Total??summary.Responses??summary.responses??a.population?.rows??state.base?.rows),target=num(state.rules.target),score=num(summary.CSAT??summary.csat??summary.overallCSAT??summary.score??a.overall?.csat);
 const buckets=csatBucketCountsFromSummary(summary,a.counts||{},total),good=num(buckets.satisfied),bad=num(buckets.dissatisfied),share=num(buckets.satisfiedPct);
 const scoreRows=analysisPeriods().map(function(r){return {period:rowPeriodName(r),score:num(r.CSAT??r.csat??r.mean??r.Mean??r.score??r.Score),responses:rowResponses(r),negative:num(r.Negative??r.negative??r.NegativeSentiment??r.negativeSentiment)}}).filter(function(r){return Number.isFinite(r.score)});
 ...
 const agentScores=(a.agents||[]).map(function(r){return num(r['Agent CSAT']??r.CSAT??r.csat??r.score??r.Score)}).filter(Number.isFinite),managerScores=(a.managers||[]).map(function(r){return num(r['Manager CSAT']??r.CSAT??r.csat??r.score??r.Score)}).filter(Number.isFinite);
 ...
 const gap=Number.isFinite(score)&&Number.isFinite(target)?score-target:NaN,firstRow=scoreRows[0],prev2=scoreRows.at(-3),last8Rows=scoreRows.slice(-8),previous4Rows=scoreRows.slice(-8,-4),targetMixThreshold=80;
 ...
 const targetExtraRows=[
  {cat:'Target',q:'Are we at least 5 points above the CSAT target?',ok:Number.isFinite(gap)&&gap>=5,unknown:!Number.isFinite(gap),basis:'Checks whether current performance has meaningful headroom above target.',formula:'Current CSAT - Target >= 5',values:[{Metric:'Current CSAT',Value:score},{Metric:'Target',Value:target},{Metric:'Gap',Value:gap},{Metric:'Required gap',Value:5}]},
  {cat:'Target',q:'If below target, is the miss within 2 points?',ok:Number.isFinite(gap)&&gap>=-2,unknown:!Number.isFinite(gap),basis:'Checks whether the current miss is small enough to be treated as near-target.',formula:'Current CSAT - Target >= -2',values:[{Metric:'Target gap',Value:gap},{Metric:'Near-target tolerance',Value:-2}]},
 ...
 const momentumExtraRows=[
  {cat:'Momentum',q:'Has CSAT improved over the last 2 weeks?',ok:latest&&prev2&&latest.score>prev2.score,unknown:!latest||!prev2,basis:'Compares the latest period with two weeks ago.',formula:'Latest CSAT > CSAT two periods ago',values:latest&&prev2?[{Period:'Latest','CSAT':latest.score},{Period:'Two periods ago','CSAT':prev2.score},{Metric:'Change',Value:latest.score-prev2.score}]:[]},
  {cat:'Momentum',q:'Has CSAT improved over the last 3 weeks?',ok:latest&&scoreRows.at(-4)&&latest.score>scoreRows.at(-4).score,unknown:!latest||!scoreRows.at(-4),basis:'Compares the latest period with three periods ago.',formula:'Latest CSAT > CSAT three periods ago',values:latest&&scoreRows.at(-4)?[{Period:'Latest','CSAT':latest.score},{Period:'Three periods ago','CSAT':scoreRows.at(-4).score},{Metric:'Change',Value:latest.score-scoreRows.at(-4).score}]:[]},
  {cat:'Momentum',q:'Are the last 3 weeks non-declining?',ok:scoreRows.slice(-3).length>=3&&scoreRows.slice(-3).every(function(r,i,arr){return i===0||r.score>=arr[i-1].score}),unknown:scoreRows.length<3,basis:'Checks whether each of the last three periods held or improved.',formula:'Week 2 >= Week 1 AND Week 3 >= Week 2',values:scoreRows.slice(-3).map(function(r){return {Period:r.period,'CSAT':r.score}})},
  ...
  {cat:'Momentum',q:'Is latest CSAT above the first available period?',ok:latest&&firstRow&&latest.score>firstRow.score,unknown:!latest||!firstRow,basis:'Checks whether performance improved across the selected range.',formula:'Latest CSAT > first available CSAT',values:latest&&firstRow?[{Period:'First',Label:firstRow.period,'CSAT':firstRow.score},{Period:'Latest',Label:latest.period,'CSAT':latest.score},{Metric:'Change',Value:latest.score-firstRow.score}]:[]},
  ...
  {cat:'Momentum',q:'Is latest CSAT within 2 points of the recent best?',ok:latest&&bestRecent&&bestRecent.score-latest.score<=2,unknown:!latest||!bestRecent,basis:'Checks whether current performance is close to the best recent level.',formula:'Best recent CSAT - latest CSAT <= 2',values:latest&&bestRecent?[{Period:'Recent best',Label:bestRecent.period,'CSAT':bestRecent.score},{Period:'Latest',Label:latest.period,'CSAT':latest.score},{Metric:'Gap to recent best',Value:bestRecent.score-latest.score}]:[]}
 ];
 const rows=[
  ...buildTargetDecisionRows('CSAT',scoreRows,score,target,total,minSample),
  ...buildMomentumDecisionRows('CSAT',scoreRows,target,total,minSample),
  {cat:'Risk',q:'Is the Dissatisfied share under control?',ok:Number.isFinite(negativeShare)&&negativeShare<=25,unknown:!Number.isFinite(negativeShare),basis:'Checks whether the negative outcome band is below a practical risk threshold.',formula:'Dissatisfied share <= 25%',values:[{Metric:'Dissatisfied',Value:bad},{Metric:'Total responses',Value:total},{Metric:'Dissatisfied share %',Value:negativeShare},{Metric:'Threshold %',Value:25}]},
  ...
  {cat:'Stability',q:'Is CSAT stable week to week?',ok:volatility<=(3),unknown:!Number.isFinite(volatility)||scoreRows.length<2,basis:'Uses period-level standard deviation as the operating stability signal.',formula:'Period standard deviation <= 3',values:[{Metric:'Standard deviation',Value:volatility},{Metric:'Periods available',Value:scoreRows.length},{Metric:'Threshold',Value:3}]},
  ...
  {cat:'People',q:'Is agent performance spread controlled?',ok:agentSpread<=10,unknown:!Number.isFinite(agentSpread),basis:'Reveals hidden variation between strongest and weakest agent outcomes.',formula:'Max agent CSAT - Min agent CSAT <= 10 pts',values:[{Metric:'Agent score spread',Value:agentSpread},{Metric:'Threshold',Value:10},{Metric:'Agents available',Value:agentScores.length}]},
  {cat:'People',q:'Is manager/team spread controlled?',ok:managerSpread<=8,unknown:!Number.isFinite(managerSpread),basis:'Reveals hidden variation between strongest and weakest manager/team outcomes.',formula:'Max manager CSAT - Min manager CSAT <= 8 pts',values:[{Metric:'Manager score spread',Value:managerSpread},{Metric:'Threshold',Value:8},{Metric:'Managers available',Value:managerScores.length}]},
  ...
  {cat:'Correlation',q:'Is CSAT correlated with volume?',ok:Number.isFinite(volumeCorr)&&Math.abs(volumeCorr)>=0.4,unknown:!Number.isFinite(volumeCorr),basis:'Checks whether response volume and score move together strongly enough to matter.',formula:'Absolute Pearson correlation between period volume and CSAT >= 0.40',values:[{Metric:'Correlation',Value:volumeCorr},{Metric:'Operational threshold',Value:0.4},{Metric:'Periods used',Value:scoreRows.length}]},
  {cat:'Correlation',q:'Does negative sentiment move against CSAT?',ok:Number.isFinite(negativeCorr)&&negativeCorr<0,unknown:!Number.isFinite(negativeCorr),basis:'Checks whether higher negative sentiment is associated with lower score.',formula:'Pearson correlation between negative sentiment and CSAT < 0',values:[{Metric:'Correlation',Value:negativeCorr},{Metric:'Expected direction',Value:'Negative'}]},
  ...
  {cat:'Volume',q:'Did volume rise while CSAT fell?',ok:latest&&prev&&Number.isFinite(latestVolumeChange)&&Number.isFinite(movement)&&latestVolumeChange>0&&movement<0,unknown:!latest||!prev||!Number.isFinite(latestVolumeChange)||!Number.isFinite(movement),basis:'Flags the operational pattern where more responses arrived but the score declined.',formula:'Latest responses > previous responses AND latest CSAT < previous CSAT',values:latest&&prev?[{Period:'Latest',Responses:latest.responses,'CSAT':latest.score},{Period:'Previous',Responses:prev.responses,'CSAT':prev.score},{Metric:'Volume change',Value:latestVolumeChange},{Metric:'Score movement',Value:movement}]:[]}
 ];
...
function showScoreBriefingEvidence(index){const answer=state.answers[index];if(!answer)return;const summary=state.analysis?.summary||{},counts=state.analysis?.counts||{};$('evidenceStatus').textContent=`RESULTS INTERPRETATION ${answer.number||index+1} - EXECUTIVE READOUT`;$('evidenceTitle').textContent=answer.question;$('evidenceAnswer').textContent=hasUsableDetail(answer)?answer.text:'This result does not have enough mapped evidence for a confident executive interpretation.';$('evidenceMethod').textContent='Results interpretation converts the completed CSAT analysis into an executive explanation of score meaning, movement, risk, and recommended attention.';$('evidenceDerived').textContent=derivedResult(answer);$('evidenceLogic').textContent=answer.logic||'Interpret the completed score result in business language, then connect it to movement, gap, sample size, and operational action.';$('evidenceStatistics').textContent=answer.statistics||`CSAT ${formatMetric(summary.csat ?? summary.score ?? 0)} | Total responses ${Number(summary.total || counts.total || state.base?.rows || 0).toLocaleString()}`;$('evidenceGuardrail').textContent=answer.guardrail||'Use as an executive readout. For audit-level rows and exact calculations, use the Results tab.';$('evidenceTable').innerHTML=`<div class="briefing-evidence-note"><h3 class="evidence-data-title">How to read this interpretation</h3><p>The interpretation is intentionally executive-friendly. It uses the same calculated result, but frames it as what changed, why it matters, and what leaders should review next. The Results tab remains the calculation and row-evidence view.</p></div>`;$('evidenceDialog').showModal()}
...
$('baseInput').onchange=e=>upload('base',e.target.files[0]);$('lookupInput').onchange=e=>upload('lookup',e.target.files[0]);if($('helpButton'))$('helpButton').onclick=()=>$('helpDialog').showModal();$('helpClose').onclick=()=>$('helpDialog').close();$('evidenceClose').onclick=()=>$('evidenceDialog').close();const guideControl=$('guideAvatar');if(guideControl){guideControl.setAttribute('role','button');guideControl.setAttribute('tabindex','0');guideControl.setAttribute('aria-label','Show or hide Signal Guide');guideControl.addEventListener('click',toggleGuide);guideControl.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleGuide()}})}if(new URLSearchParams(window.location.search).get('resume')==='results')restoreCompletedAnalysis();else showWelcome();
