(() => {
  ...
  const outcomeValue = (row) => {
    const label = String(row?.["CSAT Type"] || row?.["NPS Type"] || "").trim().toLowerCase();
    return label === "satisfied" || label === "promoter" ? 100 : ["neutral", "dissatisfied", "passive", "detractor"].includes(label) ? 0 : null;
  };
